...
//
// Version 1.7  Updated 4/3/2018
//
//      - Minor changes.  Some comments were changed.  The 'sensor_data' 
//        variable that was locally declared in 'CBOT_main()' has been moved
//        as a global since '(motor) action' was also global anyways.  Also
//        added a "Pixy open success!" string to make it clear that it has
//        succeeded.  Also the GREEN LED was being used in two places
//        simultaneously.  Now the RED LED is used by the Pixy test function
//        so these don't interfere with the present usage of the GREEN LED.
//
...
volatile SENSOR_DATA sensor_data; // This variable holds user-defined variables
                                  //  for any sensor attachments.
...
void info_display( volatile MOTOR_ACTION *pAction )
{
    ...
    if ( ( pAction->state != previous_state ) || ( pAction->state == STARTUP ) )
    {
        ...
        switch( pAction->state )
        {
			...
			case OBJECT_FOLLOWING: 
		
				LCD_printf("Following object!\n");
...
void pixy_test_display( volatile SENSOR_DATA *pSensors )
{
    ...
    // Add a little delay, so we can actually see this on the LCD
...
void IR_sense( volatile SENSOR_DATA *pSensors, TIMER16 interval_ms )
{
    ...
    else
    {
        ...
        if ( TIMER_ALARM( sense_timer ) )
        {
            ...
            // NOTE: Just as a 'debugging' feature, let's also toggle the 
            //       green LED to know that this is working for sure.  The LED 
            //       will only toggle when the timer alarm triggers.  You should
            //       comment this out if you intend to use the GREEN LED for
            //       other tasks.  Again, it's here for *just* debugging!
                   ...
                   volatile SENSOR_DATA *pSensors )
{
    ...
    if( PIXY_has_data() )
    {
		pAction->state = OBJECT_FOLLOWING;
		...
		switch( pSensors->pixy_data.signum )
		{
			...
			if( pSensors->pixy_data.pos.x > 160 )
			{
				pAction->speed_L = (2*(pSensors->pixy_data.pos.x - 160)) + 2*( pSensors->pixy_data.pos.y );
				pAction->speed_R = 2*( pSensors->pixy_data.pos.y );
				
			}
			else
			{
				pAction->speed_L = 2*( pSensors->pixy_data.pos.y );
				pAction->speed_R = (2*(160 - pSensors->pixy_data.pos.x)) + 2*( pSensors->pixy_data.pos.y );
			...
			default: 
				LCD_printf_RC(0,0, "Unknown Pixy");
			//Do nothing
		}
...
void CBOT_main( void )
{
    ...
    if ( PIXY_open() == SUBSYS_OPEN )
    {
        ...
        // Give indication this succeeded.
        LCD_printf( "Pixy open success!\n" );

        // Wait 1 second so we can see the string displayed.
        TMRSRVC_delay_sec(1);
    ...
    else
    {
        ...
        // 'Hang' here forever, since there's nothing else we can do.
        while( 1 );
